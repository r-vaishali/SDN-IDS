...
def _handle_flowstats_received (event):
  ...
  for f in event.stats:
    if f.match.tp_dst==80:
      if str(f.match.nw_src) in web_packet:
        last_Packet_count= web_packet[str(f.match.nw_src)]
        last_byte_count=web_packet[str(f.match.nw_src)]
      web_packet[str(f.match.nw_src)] = last_Packet_count + f.packet_count
      web_byte[str(f.match.nw_src)] = last_byte_count + f.byte_count
      last_Packet_count=0
      last_byte_count=0
      send_packet(event,str(f.match.nw_src),web_packet,web_byte)  

def send_packet(event,src_ip,web_packet,web_byte):
  global srcip_count
  global srcby_count
  global srcip_blocked
  global rate_limit
  global srcip_count_blocked
  skip_loop=0
  if srcip_blocked:
  ...
  if skip_loop==0:
    ...
    srcby_count[src_ip]=web_byte[src_ip]
    global tmp_src_ip
    tmp_src_ip=None
...
# If an user exceeds the threshold
    for i in srcip_count:
      if srcip_count1[i]!='Blocked':
        smart=[[srcip_count1[i],srcby_count[i]]]
        result = loaded_model.predict(smart)
        res=result[0]
        if res>0 or srcip_count1[i]>rate_limit:
          tmp_src_ip=i
...
# Add a flow to block the user
    ...
    srcip_count_blocked[src_ip]=web_packet[src_ip]
        
    for j in srcip_count_blocked :
      #srcip_cou[src_ip]=web_packet[src_ip]
      if srcip_count_blocked[j]<rate_limit and j in srcip_blocked:   
         srcip_blocked.pop(src_ip)
